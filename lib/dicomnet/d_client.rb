module DICOMNET
  ...
  class DClient
    ...
    #
    # @param [Array<String, DICOM::DObject>, String, DICOM::DObject] files a single file path or an array of paths, alternatively a DObject or an array of DObject instances
    ...
    def load_files(files_or_objects)
      ...
      files_or_objects.each do |file_or_object|
        if file_or_object.is_a?(String)
          dcm = DICOM::DObject.read(file_or_object)
          if dcm.read_success
        ...
        elsif file_or_object.is_a?(DICOM::DObject)
    ...
    def process_presentation_context_response(presentation_contexts)
      ...
      if rejected.length == 0
        ...
        @approved_syntaxes.each_pair do |key, value|
          sntx_k = (DICOM::LIBRARY.uid(key) ? DICOM::LIBRARY.uid(key).name : 'Unknown UID!')
          sntx_v = (DICOM::LIBRARY.uid(value[1]) ? DICOM::LIBRARY.uid(value[1]).name : 'Unknown UID!')
